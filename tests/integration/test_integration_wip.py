...
class TestWIPDeposit:
    def test_deposit(self, story_client):
        ...
        balance_before = story_client.getWalletBalance()
        wip_before = story_client.WIP.balanceOf(wallet_address)
        ...
        balance_after = story_client.getWalletBalance()
        wip_after = story_client.WIP.balanceOf(wallet_address)

        # Verify WIP balance increased by deposit amount
        assert wip_after == wip_before + ip_amt
        
        # Calculate gas cost
        tx_receipt = web3.eth.wait_for_transaction_receipt(response["txHash"], timeout=300)
...
class TestWIPTransfer:
    def test_transfer(self, story_client):
        ...
        sender_wip_before = story_client.WIP.balanceOf(wallet_address)
        receiver_wip_before = story_client.WIP.balanceOf(wallet_address_2)
        ...
        sender_wip_after = story_client.WIP.balanceOf(wallet_address)
        receiver_wip_after = story_client.WIP.balanceOf(wallet_address_2)
